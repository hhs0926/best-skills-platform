<x-app-layout :title="'首页'">
    ...
    <section class="max-w-7xl mx-auto py-12 text-center">
        ...
        <div class="flex items-center justify-center gap-4 flex-wrap">
            <a href="{{ route('skills.index') }}" class="glow-btn text-white px-8 py-3 rounded-xl font-medium hover:scale-105 transition-transform shadow-lg shadow-violet-500/25">
                🔍 探索技能库
            </a>
            <a href="{{ route('register') }}" class="px-8 py-3 rounded-xl font-medium border border-white/10 hover:border-violet-500/50 hover:bg-white/5 transition-all">
                ✨ 加入社区
            </a>
        </div>
    </section>
    ...
    <section class="max-w-3xl mx-auto py-10">
        <div class="grid grid-cols-3 gap-8 text-center">
            <div>
                <div class="text-3xl sm:text-4xl font-bold text-white mb-1">{{ $stats['tested_count'] }}<span class="text-lg text-violet-400">+</span></div>
                <div class="text-sm text-gray-500">已实测</div>
            </div>
            <div>
                <div class="text-3xl sm:text-4xl font-bold text-white mb-1">{{ $featuredSkills->count() }}</div>
    ...
    <section class="max-w-7xl mx-auto mb-16">
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('skills.index') }}" class="text-sm text-violet-400 hover:text-violet-300 transition">查看全部 →</a>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach($featuredSkills as $skill)
                <a href="{{ route('skills.show', $skill) }}" class="glass-card p-5 block">
                    <div class="flex items-start gap-4">
                        <div class="skill-icon">{{ $skill->icon }}</div>
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-white mb-1 truncate">{{ $skill->name }}</h3>
                            <p class="text-gray-400 text-sm line-clamp-2 mb-3">{{ $skill->short_desc }}</p>
                            <div class="flex items-center gap-3 text-xs text-gray-500">
                                <span class="flex items-center gap-1"><span class="rating-star">★</span> {{ $skill->avg_rating }}</span>
                                <span>↓ {{ number_format($skill->install_count) }} 安装</span>
                                <span>❤️ {{ number_format($skill->likes_count) }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="mt-3 pt-3 border-t border-white/5 flex items-center gap-2">
                        <span class="tag-pill">{{ $categoryLabels[$skill->category] ?? $skill->category }}</span>
