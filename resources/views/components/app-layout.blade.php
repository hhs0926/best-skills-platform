    ...
    <nav class="nav-blur fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 py-3.5">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-6 sm:gap-8">
                <a href="{{ route('home') }}" class="flex items-center gap-2 text-lg sm:text-xl font-bold no-underline hover:opacity-90">
                    <span class="text-xl sm:text-2xl">⚡</span>
                    <span class="gradient-text">BestSkills</span>
                </a>
                <div class="hidden md:flex items-center gap-6 text-sm text-gray-400">
                    <a href="{{ route('home') }}" class="hover:text-white transition {{ request()->is('/') ? 'text-white font-medium' : '' }}">首页</a>
                    <a href="{{ route('skills.index') }}" class="hover:text-white transition {{ request()->is('skills*') && !request()->is('skills/*') ? 'text-violet-400 font-medium' : '' }}">技能库</a>
                    <a href="{{ route('community') }}" class="hover:text-white transition {{ request()->is('community') ? 'text-violet-400 font-medium' : '' }}">社区</a>
                    <a href="{{ route('exchange') }}" class="hover:text-white transition {{ request()->is('exchange') ? 'text-violet-400 font-medium' : '' }}">交换</a>
                </div>
            </div>
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                @else
                    ...
                    <a href="{{ route('register') }}" class="glow-btn text-white px-3 sm:px-4 py-2 rounded-lg text-sm">注册</a>
                @endauth
            </div>
        </div>
    </nav>
